...
enum S3Signing {
    ...
    private static func canonicalQuery(_ query: String) -> String {
        ...
        let pairs = query.split(separator: "&").map { String($0) }.sorted()
        return pairs.joined(separator: "&")
    }
}

public final class S3CloudAdapter: ProjectCloudAdapter {
    ...
    public func testConnection() async throws {
        ...
        // HeadBucket via path-style: HEAD /{bucket}
        let url = objectURL(key: "")
        var req = URLRequest(url: url.deletingLastPathComponent().appendingPathComponent(bucket))
        // Prefer ListObjects with max-keys=0 / prefix
        let listURL = listURL()
        req = URLRequest(url: listURL)
        req.httpMethod = "GET"
    ...
    public func uploadPackage(localProjectURL: URL) async throws {
        ...
        // v1 pragmatic: upload to final keys; on failure best-effort delete incomplete prefix.
    ...
    private func ensureCredentials() throws {
        ...
        if bucket.isEmpty {
            throw ProjectCloudError.network("missing bucket")
        }
    }
    ...
    private func throwIfNeeded(_ response: URLResponse) throws {
        ...
        guard (200..<300).contains(http.statusCode) else {
            throw ProjectCloudError.server(status: http.statusCode, message: "S3 request failed")
        }
    }
}
