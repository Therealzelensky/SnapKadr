...
enum WebDAVPath {
    static func join(_ base: URL, prefix: String, components: String...) -> URL {
        ...
        for c in components where !c.isEmpty {
            url = url.appendingPathComponent(c)
        }
        return url
    }
...
public final class WebDAVCloudAdapter: ProjectCloudAdapter {
    ...
    public func uploadPackage(localProjectURL: URL) async throws {
        ...
        // Best-effort cleanup of prior incomplete upload.
        ...
        // Remove existing final if present, then MOVE temp → final.
    ...
    private func ensureCredentials() throws {
        if password.isEmpty { throw ProjectCloudError.authRequired }
    }
    ...
    private func mkcol(_ url: URL) async throws {
        ...
        if let http = response as? HTTPURLResponse {
            ...
            throw ProjectCloudError.server(status: http.statusCode, message: "MKCOL")
        }
    }
    ...
    private func throwIfNeeded(_ response: URLResponse, fallback: String) throws {
        guard let http = response as? HTTPURLResponse else {
            throw ProjectCloudError.network(fallback)
        }
        if http.statusCode == 401 || http.statusCode == 403 {
            throw ProjectCloudError.authRequired
        }
        guard (200..<300).contains(http.statusCode) || http.statusCode == 207 else {
            throw ProjectCloudError.server(status: http.statusCode, message: fallback)
        }
    }
}
