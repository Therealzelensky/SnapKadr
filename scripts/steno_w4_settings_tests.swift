...
enum StenoW4SettingsTests {
    static func main() {
        ...
        expect(settings.contains("cloud.yandex.oauthClientID"), "yandex oauth client id prefs key")
        ...
        expect(g.contains("yandexOAuthClientID") || g.contains("OAuth Client ID"), "yandex client id field")
        expect(g.contains("OAuth Client Secret") || g.contains("clientSecret"), "yandex client secret field")
        expect(g.contains("verification_code") || g.contains("oauth.yandex.ru"), "yandex connect guidance")
